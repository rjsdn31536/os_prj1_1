...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  if (!setup_stack (esp))
	  goto done;
  /* make argv full*/
  strlcpy(argv[0],f_name,strlen(f_name)+1);
  int cnt =1;
  // save argv argument
  f_name = strtok_r(NULL," ",&ptr);
  while(f_name != NULL)
  {
	  strlcpy(argv[cnt++],f_name,strlen(f_name)+1);
	  f_name = strtok_r(NULL," ",&ptr);
  }
  argc = cnt;
  int len =0;
  ...
  for(i=0;i<argc;i++)
  {
	  ...
	  total_len += len;
	  *esp -= len;
	  strlcpy((char*)*esp,argv[argc-i-1],len);
	  argv_address[argc-i-1] = *(unsigned *)esp;
  ...
  **(unsigned**)esp = 0;
  ...
  //push argv argument address
  for(i=0;i<argc;i++)
  {
	  *esp -= 4;
	  **(unsigned**)esp = argv_address[argc-i-1];
  }
